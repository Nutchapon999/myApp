...
namespace myApp.DAL
{
    public class WorkFlow
    {
        ...
        public List<WorkFlow> GetWorkflows(string username, string year)
        {
            ...
            {
                command.Connection = connection;
                command.CommandText = "SELECT *, RE.K2_NO AS RESULT_K2 " +
                                    "FROM WORKFLOW_HISTORY WF " +
                                    "JOIN REMARK_HISTORY RM ON WF.REMARK_ID = RM.ID " +
                                    "JOIN IDP_RESULT RE ON RM.FORM_GUID = RE.GUID " +
                                    "WHERE WF.REMARK_ID IS NOT NULL AND WF.ACTION_BY = @Username AND RE.YEAR = @Year";
                ...
                using (SqlDataReader reader = command.ExecuteReader())
                {
                    ...
                    {
                        WorkFlow workFlow = new WorkFlow();
                        workFlow.K2_NO = (string)reader["RESULT_K2"];
                        workFlow.Action = (string)reader["ACTION_BY"];
                        workFlow.Subject = (string)reader["SUBJECT"];
